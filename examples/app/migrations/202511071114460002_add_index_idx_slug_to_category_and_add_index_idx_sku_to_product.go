...
type AddIndexIdxSlugToCategoryAndAddIndexIdxSkuToProduct struct{}

func (m AddIndexIdxSlugToCategoryAndAddIndexIdxSkuToProduct) Version() string { return "202511071114460002" }

func (m AddIndexIdxSlugToCategoryAndAddIndexIdxSkuToProduct) Name() string { return "add_index_idx_slug_to_category_and_add_index_idx_sku_to_product" }

func (m AddIndexIdxSlugToCategoryAndAddIndexIdxSkuToProduct) Up(db *gorm.DB) error {
...
func (m AddIndexIdxSlugToCategoryAndAddIndexIdxSkuToProduct) Down(db *gorm.DB) error {
	if sim, ok := any(db).(*goosegorm.SchemaBuilder); ok {
...
func init() {
	goosegorm.RegisterMigration(AddIndexIdxSlugToCategoryAndAddIndexIdxSkuToProduct{})
}
